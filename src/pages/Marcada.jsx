...
function calcPrendas(result, talles, piezasReq) {
  const reqMap = {}
  ...
  for (const t of talles) {
    if (reqKeys.length > 0) {
      // iterate over ALL required pieces — missing ones contribute 0 cuts
...
function TablaPorPar({ marcadaId, productoId, talles, piezas, pares, ajustes, onReload, catalogPiezas, piezasReq }) {
  ...
  // include required pieces (piezasReq) even if not yet cut
    ...
    <div style={{ overflowX:'auto', marginBottom:8 }}>
      ...
      <div style={{ display:'flex', gap:16, alignItems:'flex-start', flexWrap:'wrap' }}>
        ...
        {piezaNames.length > 0 && pares > 0 && (() => {
          ...
          // collect all piezas including those created by modifications
...
function ResumenCortesSection({ entries, allPiezas, allAjustes, allPiezasReq, pedido, talles, productoId, marcadaId }) {
  ...
  // cant_prenda por pieza (de cualquier entrada)
  const reqMap = {} // pieza_nombre -> cant_prenda
  ...
  // cortes totales por pieza+talle (sumando todas las marcadas)
  const cortadoMap = {} // pieza -> talle -> qty
  for (const { marcada, prodEntry } of entries) {
  ...
  // each pieza column = 3 sub-columns: precisan · cortaron · diff
  return (
      ...
      {!collapsed && (
        <div style={{ padding:'6px 8px', overflowX:'auto' }}>
          <table style={{ ...S.tbl, tableLayout:'auto' }}>
            <thead>
              {/* Row 1: Talle header + pieza names (each spanning 3 sub-cols) */}
              <tr>
...
// ── MarcadaSlim ───────────────────────────────────────────────────────────────
// Marcada config + TablaPorPar + ModificacionesSection for one product
...
// ── TelaSubSection ────────────────────────────────────────────────────────────
// Tela header + PiezasPorPrenda + MarcadaSlim list — used inside ProductoSection
...
function TelaSubSection({ tela, items, prod, talles, allPiezas, allAjustes, allPiezasReq, onDeleteMarcada, onReload, sessionId, productoId }) {
  ...
  // use the first prodEntry for PiezasPorPrendaSection (pieces are per tela, shared)
...
function MarcadaBlock({ marcada, num, allPiezas, allAjustes, allPiezasReq, onDeleteMarcada, onReload }) {
  const [collapsed,  setCollapsed]  = useState(false)
  ...
  const [cfg, setCfg] = useState({
    ...
    metros_reales:  String(marcada.metros_reales || ''),
    nota:           marcada.nota || '',
  })
  const prodTimer = useRef(null)
  const saveTimer = useRef(null)

  const prods  = marcada.marcadas_sim_productos || []
  const pares  = (parseFloat(cfg.total_pliegues)||0) / (parseFloat(cfg.pliegues)||1)
  const metrosT= (parseFloat(cfg.metros)||0) * (parseFloat(cfg.total_pliegues)||0)
  ...
  function onCfgChange(k, v) {
    ...
    saveTimer.current = setTimeout(async () => {
      await supabase.from('marcadas_sim').update({
        ...
        total_pliegues: parseFloat(next.total_pliegues) || 0,
        metros_reales:  parseFloat(next.metros_reales) || null,
  ...
  function onProdInput(val) {
    ...
    prodTimer.current = setTimeout(async () => {
      const { data } = await supabase.from('productos')
        .select('id, nombre, tabla, cara_uso, piezas')
  ...
  async function addProd(p) {
    ...
    await supabase.from('marcadas_sim_productos').insert({ marcada_id: marcada.id, producto_id: p.id })
  ...
  async function delProd(id) {
    ...
    await supabase.from('marcadas_sim_productos').delete().eq('id', id)
    onReload()
  }
    ...
    <div style={{ border:'1px solid #d8d8d0', marginBottom:8, background:'#fafafa' }}>
      <div style={S.marcHead}>
        ...
        <span style={{ color:'#555' }}>
          {cfg.metros||'0'}m · {cfg.pliegues||1}pp/par · {cfg.total_pliegues||0} total
          {' = '}<b style={{ color:'#1a3a6b' }}>{isNaN(pares)||!isFinite(pares) ? '—' : fmtN(pares,1)} pares</b>
          {' · '}<b>{fmtN(metrosT)}m</b> calculados
          {cfg.metros_reales && <> · <b style={{ color:'#1a6a1a' }}>{cfg.metros_reales}m</b> reales</>}
        </span>
      ...
      {!collapsed && (
        <div style={{ padding:'8px' }}>
          ...
          <div style={{ display:'flex', gap:10, alignItems:'flex-end', padding:'8px 10px', background:'#f4f8fc', border:'1px solid #c8d8e8', marginBottom:8, flexWrap:'wrap' }}>
            ...
            <div>
              <span style={S.lbl}>METROS CALCULADOS</span>
              <div style={{ fontSize:13, fontWeight:700, color:'#1a3a6b', paddingBottom:2 }}>{fmtN(metrosT)} m</div>
            </div>
            <div>
              <span style={{ ...S.lbl, color:'#1a6a1a' }}>METROS REALES 🖥</span>
              <input style={{ ...S.inp, ...S.inpR, width:70 }} type="number" step={0.01}
                value={cfg.metros_reales} onChange={e => onCfgChange('metros_reales', e.target.value)}
            ...
            <div style={{ padding:'4px 0' }}>
              <div style={{ position:'relative', display:'flex', gap:6, alignItems:'center' }}>
                <input style={{ ...S.inp, flex:1 }} placeholder="Buscar producto..."
                  value={prodQ} onChange={e => onProdInput(e.target.value)} autoFocus />
                <button style={S.btn} onClick={() => { setAddingProd(false); setProdQ(''); setProdRes([]) }}>✕</button>
                <AcList items={prodRes} onPick={addProd} label={p => `${p.nombre} [${p.tabla}]`} />
              </div>
            </div>
          ) : (
...
function TelaSection({ tela, marcadas, allPiezas, allAjustes, allPiezasReq, onDeleteMarcada, onReload, sessionId }) {
  const [collapsed, setCollapsed] = useState(false)
  const [adding,    setAdding]    = useState(false)
  const rol        = marcadas[0]?.rol || ''
  const telaName   = tela ? telaLabel(tela) : ''
  const label      = rol ? (ROL_LABELS[rol] || rol) : (telaName || 'Tela 1')
  const totalCalc  = marcadas.reduce((s, m) => s + calcMetrosTotal(m), 0)
  const totalReales = marcadas.reduce((s, m) => s + (parseFloat(m.metros_reales) || 0), 0)

  async function setRol(val) {
    for (const m of marcadas) {
      await supabase.from('marcadas_sim').update({ rol: val || null }).eq('id', m.id)
    }
    onReload()
  }
  ...
  async function addMarcada() {
    setAdding(true)
    const { data:{ user } } = await supabase.auth.getUser()
    await supabase.from('marcadas_sim').insert({
      session_id: sessionId, tela_id: tela?.id || null,
    ...
    <div style={S.secWrap}>
      <div style={S.telaHead}>
        <span style={{ cursor:'pointer', flex:1 }} onClick={() => setCollapsed(c => !c)}>
          {collapsed ? '▶' : '▼'} 🧵 {label}
          {telaName && rol && <span style={{ fontWeight:400, color:'#777', marginLeft:6, fontSize:10 }}>({telaName})</span>}
          <span style={{ fontWeight:400, color:'#555', marginLeft:8, fontSize:10 }}>
            {marcadas.length} marcada{marcadas.length !== 1 ? 's' : ''} · {fmtN(totalCalc)} m calc.
            {totalReales > 0 && <span style={{ color:'#1a6a1a', marginLeft:4 }}>· {fmtN(totalReales)} m reales</span>}
          </span>
        </span>
        <div style={{ display:'flex', gap:6, alignItems:'center' }}>
          <select style={{ ...S.sel, fontSize:10 }} value={rol} onClick={e => e.stopPropagation()} onChange={e => setRol(e.target.value)}>
            <option value="">— rol —</option>
            {Object.entries(ROL_LABELS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
          </select>
